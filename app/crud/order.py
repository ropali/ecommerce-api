...
def cancel_order(db: Session, order_id: int) -> Order:
    """
    Cancel an order and return products to stock.
    ...
    Raises:
        OrderNotFoundException: If order with given ID doesn't exist
        InvalidOrderDataException: If order is already completed
    """
    db_order = get_order(db, order_id)
    ...
    if db_order.status != OrderStatus.PENDING.value:
        raise InvalidOrderDataException(
            f"Cannot cancel order with status: {db_order.status}"
        )
    ...
    # Mark as canceled (using completed with a negative total price as a flag)
    db_order.status = OrderStatus.COMPLETED.value
    db_order.total_price = -db_order.total_price  # Negative price indicates canceled
